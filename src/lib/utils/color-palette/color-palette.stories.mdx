...
const { getFullColorPaletteForVariable } = require('antd-react-extensions/dist/utils/color-palette/color-palette-node')

postcss: {
  plugins: [
    replaceValuesPlugin({
      values: {
        ...getFullColorPaletteForVariable('#1890ff', 'primary-color'),
        ...getFullColorPaletteForVariable('#ff4d4f', 'danger-color')
      }
    })
  ]
}
